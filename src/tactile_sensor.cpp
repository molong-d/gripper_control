...
void TactileSensor::start_reading() {
    ...
    if (!sensor_timer_) {
        ...
        sensor_timer_ = node_->create_wall_timer(
            50ms, std::bind(&TactileSensor::read_sensor_data, this)  // 调整为50ms间隔，20Hz频率
        );
...
bool TactileSensor::send_sensor_command_and_read_response(const std::vector<uint8_t>& send_buf, 
    ...
    // 等待响应，减少等待时间以提高响应速度
    usleep(20000);  // 20ms instead of 100ms
...
bool TactileSensor::select_sensor_port(uint8_t port_id) {
    ...
    std::vector<uint8_t> response;
    usleep(100000);  // 减少端口切换延迟从1秒到100ms
    if (!send_sensor_command_and_read_response(cmd, response)) {
...
bool TactileSensor::read_sensor_raw_data(uint8_t port_id, std::vector<uint8_t>& data) {
    ...
    std::vector<uint8_t> response;
    usleep(10000);  // 减少数据读取延迟从50ms到10ms
...
bool TactileSensor::read_sensor(uint8_t sensor_id, uint8_t port_id, float& fx, float& fy, float& fz) {
    ...
    if (parse_force_data(raw_data, fx, fy, fz)) {
        // printf("Time:[%.3f]",node_->now().seconds());

        RCLCPP_INFO(node_->get_logger(), "传感器%d(端口%d): FX=%.1fN, FY=%.1fN, FZ=%.1fN",
                    sensor_id, port_id, fx, fy, fz);
        return true;
...
void TactileSensor::read_sensor_data() {
    ...
    if (read_sensor(SENSOR1_ID, SENSOR1_PORT, fx, fy, fz)) {
        ...
        // 更新数据缓存
        s1_fx_history_.push_back(fx);
        s1_fy_history_.push_back(fy);
        s1_fz_history_.push_back(fz);
        
        // 保持缓存大小
        if (s1_fx_history_.size() > HISTORY_SIZE) {
            s1_fx_history_.pop_front();
            s1_fy_history_.pop_front();
            s1_fz_history_.pop_front();
        }
    }
    ...
    if (read_sensor(SENSOR2_ID, SENSOR2_PORT, fx, fy, fz)) {
        ...
        // 更新数据缓存
        s2_fx_history_.push_back(fx);
        s2_fy_history_.push_back(fy);
        s2_fz_history_.push_back(fz);
        
        // 保持缓存大小
        if (s2_fx_history_.size() > HISTORY_SIZE) {
            s2_fx_history_.pop_front();
            s2_fy_history_.pop_front();
            s2_fz_history_.pop_front();
        }
    }
}
...
// 滤波方法实现
float TactileSensor::get_filtered_s1_fx() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s1_fx_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s1_fx_history_) sum += val;
        return sum / s1_fx_history_.size();
    }
    return s1_fx_;
}

float TactileSensor::get_filtered_s1_fy() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s1_fy_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s1_fy_history_) sum += val;
        return sum / s1_fy_history_.size();
    }
    return s1_fy_;
}

float TactileSensor::get_filtered_s1_fz() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s1_fz_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s1_fz_history_) sum += val;
        return sum / s1_fz_history_.size();
    }
    return s1_fz_;
}

float TactileSensor::get_filtered_s2_fx() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s2_fx_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s2_fx_history_) sum += val;
        return sum / s2_fx_history_.size();
    }
    return s2_fx_;
}

float TactileSensor::get_filtered_s2_fy() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s2_fy_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s2_fy_history_) sum += val;
        return sum / s2_fy_history_.size();
    }
    return s2_fy_;
}

float TactileSensor::get_filtered_s2_fz() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (s2_fz_history_.size() >= HISTORY_SIZE) {
        float sum = 0;
        for (auto val : s2_fz_history_) sum += val;
        return sum / s2_fz_history_.size();
    }
    return s2_fz_;
}
